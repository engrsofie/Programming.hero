		    ...
		    <div class="col-md-12">
                <div class="panel panel-inverse">
                    ...
                    <div class="panel-body">
                        {!! Form::open(array('route' => ['deposit.update',$getDepositData->id],'class'=>'form-horizontal author_form','method'=>'PUT','files'=>'true', 'id'=>'commentForm','role'=>'form','data-parsley-validate novalidate')) !!}
                        	<div class="form-group">
								
								<input class="form-control" type="hidden" id="client" value="<?php echo $getDepositData->company_name; ?>" name="company_name" placeholder="Company Name" data-parsley-required="true" />
							</div>
                                    ...
                                    <div class="col-md-6 col-sm-6">
                                        <select name="fk_client_id" data-placeholder="- Select Client-" class="select form-control" tabindex="10" required="required">
                                            @foreach($getClientData as $client)
                                                <option value="{{$client->id}}" @if($client->id == $getDepositData->fk_client_id){{ "selected" }} @endif>{{$client->client_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6">
                                        <input class="form-control" type="number" id="number" value="<?php echo $getDepositData->ref_id;?>" name="ref_id" data-parsley-type="number" placeholder="Ref. Id" />
                                    </div>
                                    <input type="hidden" name="invoice_no" value="<?php echo $getDepositData->invoice_no;?>">
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6">
                                        <select name="fk_account_id" data-placeholder="- Select account-" class="chosen-select-account form-control" tabindex="10" required="required">
                                            ...
                                            <option value="{{$account->id}}" @if($account->id == $getDepositData->fk_account_id){{ "selected" }} @endif>{{$account->account_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6">
                                        <select name="fk_method_id" data-placeholder="- Select method-" class="chosen-select-method form-control" tabindex="10" required="required">
                                            ...
                                            <option value="{{$method->id}}" @if($method->id == $getDepositData->fk_method_id){{ "selected" }} @endif>{{$method->method_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class='row'>
                                        <div class='col-xs-12 col-sm-12 col-md-12 col-lg-12'>
                                            <table class="table table-bordered table-hover" id="table_auto">
                                                ...
                                                <tbody>
                                                @if(isset($getDepositItemData))
                                                    <?php $i=0; ?>
                                                    @foreach($getDepositItemData as $depositItem)
                                                        <?php $i++; ?>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <select name="fk_sub_category_id[]" data-placeholder="- Select -" class="chosen-select-category1 form-control" tabindex="10" required="required">
                                                                ...
                                                                foreach ($subCategories as $subCategory) {
                                                                 ?>
                                                                <option value="<?php echo $subCategory->id; ?>" @if($depositItem->fk_sub_category_id == $subCategory->id){{ "selected" }} @endif><?php echo $subCategory->sub_category_name; ?></option>
                                                                <?php } ?>
                                                            </select>
                                                        ...
                                                        <td>
                                                            ...
                                                            <input type="text" data-type="description" name="description[]" id="description_1" value="{{$depositItem->description}}" class="form-control autocomplete_txt" autocomplete="off" required>
                                                            
                                                        </td>
                                                        <td>
                                                            <input type="number" min="0" name="total[]" id="price_1" class="form-control onChangeAmount" value="{{$depositItem->total_amount}}" onKeyPress="amount()" onKeyUp="amount()">
                                                            
                                                        </td>
                                                        <td>
                                                            <input type="number" min="0" name="paid[]" id="paid_1" class="form-control onChangeAmountPaid" value="{{$depositItem->paid_amount}}"  autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;">
...
@include('deposit.deposit_js_setting_update')
