            ...
            <div class="panel panel-inverse">
                <div class="panel-heading"> 
                    <div class="panel-heading-btn">
                            <a class="btn btn-info btn-xs" href="{{URL::to('/inventory-transfer')}}">View All</a>
                            
                        </div>
                    <h4 class="panel-title">Product Transfer</h4>
                </div>
                <div class="panel-body">
                	 {!! Form::open(array('route' => 'inventory-transfer.store','class'=>'form-horizontal author_form','method'=>'POST','role'=>'form','data-parsley-validate novalidate')) !!}
                     <div class="col-md-12 no-padding">
                         <div class="form-group col-md-4">
                             <label class="col-md-12">Transfer From :</label>
                             <div class="col-md-12">
                                @if(Auth::user()->isRole('administrator'))
                                 {{Form::select('transfer_from',$branch,Auth::user()->fk_branch_id,['class'=>'form-control','id'=>'transferFrom','required'])}}
                                @else
                                    <span class="form-control">{{$ownBranch->branch_name}}</span>
                                    <input type="hidden" name="transfer_from" value="{{$ownBranch->id}}">
                                @endif
                             </div>
                         </div>
                         <div class="form-group col-md-4">
                             ...
                             <div class="col-md-12">
                                 {{Form::select('transfer_to',$branch,'',['class'=>'form-control','id'=>'transferTo','required','placeholder'=>'Select Branch'])}}
                             </div>
                         </div>
                         <div class="form-group col-md-4">
                             <label class="col-md-12">Date :</label>
                             <div class="col-md-12">
                                 <input type="text" name="date" placeholder="Date" class="form-control datepicker" value="{{date('d-m-Y')}}">
                             </div>
